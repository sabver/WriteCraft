      ...
      <MainLayout>
        <PageWrapper>
          <div className="max-w-xl mx-auto h-[60vh] flex flex-col items-center justify-center text-center space-y-8">
            ...
            <div>
              <h2 className="text-3xl font-black text-slate-900 tracking-tight mb-2 font-display">{t('allCaughtUp')}</h2>
              <p className="text-slate-500 text-lg font-medium">{t('allCaughtUpBody')}</p>
            </div>
        ...
        <div className="max-w-4xl mx-auto flex flex-col h-full">
          <header className="flex items-center justify-between mb-12">
            <div className="flex items-center gap-6">
              <Link href="/" className="text-slate-400 hover:text-slate-900 transition-colors" aria-label={t('backAria')}>
            ...
            <div className="flex flex-col items-end gap-2">
              <div className="flex items-center gap-3">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('progressLabel')}</span>
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-12 min-h-[500px]">
            ...
            <AnimatePresence mode="wait">
              {isFlipped && (
                ...
                >
                  <p className="text-center text-slate-400 text-sm font-bold uppercase tracking-widest mb-6">{t('ratePrompt')}</p>
          ...
          <div className="mt-8 flex justify-center gap-8 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
            <span className="flex items-center gap-2">
              <kbd className="bg-slate-100 px-2 py-1 rounded-md text-slate-600 font-mono">1-5</kbd>
              {t('ratePrompt')}
            </span>
            <span className="flex items-center gap-2">
              <kbd className="bg-slate-100 px-2 py-1 rounded-md text-slate-600 font-mono">␣</kbd>
              {t('flipHint')}
            </span>
          </div>
        </div>
